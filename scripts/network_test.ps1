...
try {
    ...
    $iqOK = ($code -eq 200 -or $code -eq 301 -or $code -eq 302 -or $code -eq 403)
    $resp.Close()
} catch [System.Net.WebException] {
...
Write-Host ""
if ($iqOK) {
    Write-Host "  SUCCESS: YOUR PC CAN CONNECT TO IQ OPTION!" -ForegroundColor Green
    Write-Host "  The bot should work from this machine." -ForegroundColor Green
} else {
    Write-Host "  WARNING: CONNECTION ISSUES DETECTED" -ForegroundColor Red
